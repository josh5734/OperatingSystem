...
```
- 커널(운영체제)은 User process의 일부분처럼 나타난다. OS Code가 앞에서 살펴본 프로그램 이미지(Image)와 연결되어 있다는 것이다.
- 이 방법에서 Interrupt가 일어나게 되면 CPU는 User mode에서 Kernel mode로 Mode-Switching을 하게 된다.
- 이 때, OS가 각 User process 위에 있기 때문에 Process 간 Switching이 일어날 필요가 없게 되고, 이는 곧 같은 프로세스 안에서의 mode switching만 일어나게 해서 Overhead를 감소시켜준다.
```
* 우리가 잘 알고 있는 Linux가 Execution within User process 모델을 가지고 있다. 구체적인 Linux Process Image를 보면 아래와 같고, 몇 가지 설명을 수업시간에 써넣었다.   
<img src = "https://user-images.githubusercontent.com/61929745/112716417-b7804500-8f29-11eb-9338-6771fe113cc6.png" width = 600 height = 500>    

* 위의 그림에서 Kernel의 주소 공간은 User 주소 공간의 바로 위에 1GB 크기로 자리 잡고 있다. 
* 그리고 이 윗부분의 주소 공간을 모든 Process에서 공유(dead copy)하고 있게 된다. 
* 커널이 별도의 PCB로 존재하지 않기 때문에 Mode Switching만 필요하게 되고, 따라서 Overhead가 적게 된다. 
...
```
- 여기에서는 칵 Kernel을 독립된 process로 구성하게 되고, OS를 이러한 Process들의 집합으로 보는 것이다. -
- 앞선 방식에서는 커널이 각 유저 프로세스마다 존재했고, 그 공간이 1GB정도였는데, 여기서는 각 커널이 하나의 프로세스 전체를 차지하게 되므로 4GB 전체를 쓸 수 있게 되었다.
- 또한 OS를 모듈로 나눠서 구현 가능하게 되고, 병렬 컴퓨터라면 높은 스피드를 낼 수 있게 된다.
- 하지만 무엇보다 큰 단점은 각 OS가 독립적인 프로세스로 존재하기 때문에, Mode Switching이 아니라 Process Switching이 필요하게 되고, 이에 따라 속도가 느려질 수밖에 없다.
```
---
...
### 1) pipe
* 과거에는 pipe라는 개념을 통해 연관성이 있는 두 프로세스 간 통신을 했다. 
* 하지만 pipe는 부모와 자식 프로세스 사이에서만 사용이 가능했기 때문에 이를 보완하기 위해 named pipe 혹은 FIFO를 고안했다.
* named pipe는 kernel 상에 항상 유지하는 개념이다. 하지만 named pipe 역시 단일 컴퓨터 내에서만 사용이 가능하다는 단점이 존재한다.
### 2) Shared memory, Semaphore
* 공유 메모리(Shared memory)는 두 프로세스 간 메모리를 공유하는 개념이다.
* 공유 메모리에서 일어날 수 있는 동기화 문제는 Semaphore를 통해 해결할 수 있다. 
* 하지만 이 방법들 역시 같은 호스트 내에서만 활용할 수 있다.
### 3) SOCKET(소켓)
* 최근에는 SOCKET 을 활용하기 시작했다. 
* 소켓은 컴퓨터네트워크 시간에 자세히 배운 개념이다. 각 프로세스는, 유저 내 프로세스 뿐만 아니라, 유저 간 프로세스 역시 양방향 소켓을 통해서 메시지를 주고 받을 수 있게 되었다.
* 이는 소켓이 IP Address, Port Number를 통해 송수신 대상을 특정할 수 있기 때문이다.
* 한편 소켓은 크게 TCP 혹은 UDP 전송 프로토콜을 사용하는데 간단한 특징은 아래와 같다.
