...
* Heap 메모리 영역에는 프로그램 런타임에 동적 메모리가 할당된다. C언어에서 malloc, calloc으로 Heap 영역의 메모리를 사용할 수 있다. 데이터 배열의 크기가 확실하지 않고 변동이 있을 때 Heap 영역을 활용해서 메모리를 할당한다. 단, 사용하고 난 다음에는 반드시 해제를 해야하는데 그렇지 않으면 메모리 누수가 일어날 수 있다.
