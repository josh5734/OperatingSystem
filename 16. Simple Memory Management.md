...
* 단, 지금부터 소개하는 방법은 기본적으로 **Process의 크기만큼 Physical(Main) Memory에 공간이 있어야 한다는 것을 가정한다.**
* 이후에 physical memory에 공간이 없을 때에 대한 페이지 교체 정책에 대해서 공부 할 것이다.

## 1. Fixed Partitioning

- 이 방법은 메인 메모리를 고정 사이즈 Partition으로 나누고, Partition의 size보다 작은 프로세스를 할당할 수 있도록 하는 것이다. 단 여기서, Partition의 사이즈는 모두 똑같을수도 있고 조금씩 다를 수도 있다.
- 그리고 이전에 Swapping 개념을 배운 것을 토대로 물리 메모리의 모든 Partition이 다 차있다면 OS는 swapping을 통해 기존 프로세스를 이동시키고 배치하는 등의 작업을 진행한다.
- 이 방법에서 생길 수 있는 문제점은 크게 두 가지다.
```
1. 하나의 프로세스가 하나의 Partition을 모두 차지하기 때문에, Internal fragmentation이 발생할 수 있다는 점이다.
   만약 Partition의 크기를 다르게 쪼갠다고 하더라도, Internal fragmentation이 적어질 수는 있지만 이 문제를 완벽히 해결하기는 어렵다.
2. 하나의 프로그램이 partition 크기보다 클 경우, 이 프로그램은 애초에 메모리에 할당될 수가 없다. 따라서 프로그래머는 Overlay 기법을 사용해야 한다.
```
...
### Overlay 단점
1. 프로그래머가 직접 overlap map을 짜야 한다.
2. 프로그래머가 메모리 요구 사항에 대해 알고 있어야 overlay 여부를 결정할 수 있다.
3. overlay된 module들은 서로 완벽히 disjoint해야 한다.
4. overlay 구조를 짜는 것은 상당히 복잡하고 모든 케이스에 대해 항상 가능한 것은 아니다.
---

한편 Fixed Partitioning을 통해 메모리를 관리하는 구체적인 알고리즘은 여러가지가 있을 수 있다.
...
* 따라서 Dynamic Partitioning을 할 때는 Compaction을 반드시 사용해야 한다.
...
* 버디 시스템을 쓰게 될 때 장단점은 무엇일까?
```
- 장점: Dynamic allocation에 비해 external fragmentation이 적으며, 하위 메모리부터 채워나가기 때문에 Compaction에 대한 overhead가 적다.
- 단점: 2^n 크기의 메모리를 할당하기 때문에 internal fragmentation이 있을 수 있다.
```
---

## 4. 나가면서
* 몇 가지 간단한 메모리 관리 기법과 Fragmentation, compaction, overlay 개념에 대해 공부했다.
* 다음 글에서 paging기법과 같은 다른 중요한! 메모리 관리 기법에 대해 이어서 정리하고자 한다.
